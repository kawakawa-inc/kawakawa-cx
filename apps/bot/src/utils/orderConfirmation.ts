...
/**
 * Build an embed for choosing between Order and Invoice mode
 */
export function buildModeChoiceEmbed(direction: 'buy' | 'sell'): EmbedBuilder {
  const title = direction === 'buy' ? '📦 Create Buy Order' : '📤 Create Sell Order'
  const color = direction === 'buy' ? 0x5865f2 : 0xfee75c

  // Direction-aware labels for invoice mode
  const invoiceLabel = direction === 'buy' ? '🛒 Select Sell Order' : '🛒 Select Buy Order'
  const invoiceDescription =
    direction === 'buy'
      ? "Browse and buy directly from a specific member's sell orders."
      : "Browse and sell directly to a specific member's buy orders."
    ...
    .addFields(
      {
        ...
        value:
          direction === 'buy'
            ? 'Post a buy order for items you want to purchase from other members.'
            : 'Post a sell order to list items you have available for sale.',
        inline: false,
      },
      {
        name: invoiceLabel,
      ...
      },
      {
        ...
        value:
          direction === 'buy'
            ? 'Modify or update your existing buy orders.'
            : 'Modify or update your existing sell orders.',
        inline: false,
      }
    )
    .setTimestamp()
...
/**
 * Build buttons for choosing between Order and Invoice mode
 */
export function buildModeChoiceButtons(
  customIdPrefix: string,
  direction: 'buy' | 'sell'
): ActionRowBuilder<ButtonBuilder> {
  const invoiceLabel = direction === 'buy' ? '🛒 Select Sell Order' : '🛒 Select Buy Order'
  ...
  return new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder()
      .setCustomId(`${customIdPrefix}:order`)
      .setLabel('📋 Create Order')
      .setStyle(ButtonStyle.Primary),
    new ButtonBuilder()
      .setCustomId(`${customIdPrefix}:invoice`)
      .setLabel(invoiceLabel)
      .setStyle(ButtonStyle.Success),
    new ButtonBuilder()
      .setCustomId(`${customIdPrefix}:edit_existing`)
      .setLabel('✏️ Edit Existing')
      .setStyle(ButtonStyle.Secondary),
    new ButtonBuilder()
      .setCustomId(`${customIdPrefix}:cancel`)
      .setLabel('❌ Cancel')
