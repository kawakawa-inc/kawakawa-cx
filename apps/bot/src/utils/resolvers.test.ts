...
const { mockResolveCommodity, mockResolveLocation, mockFindUserByName, mockFindUserByDiscordId } = vi.hoisted(() => ({
...
describe('resolvers', () => {
  ...
  describe('resolveLocation', () => {
    ...
    it('handles warehouse type', async () => {
      mockResolveLocation.mockResolvedValue({ naturalId: 'WH1', name: 'Warehouse 1', type: 'Warehouse' })
  ...
  describe('resolveUser', () => {
    it('returns resolved user', async () => {
      mockFindUserByName.mockResolvedValue({
        userId: 1, username: 'alice', displayName: 'Alice', fioUsername: 'AliceFIO',
      })
  ...
  describe('resolveDiscordMention', () => {
    it('returns resolved user from discord ID', async () => {
      mockFindUserByDiscordId.mockResolvedValue({
        userId: 2, username: 'bob', displayName: 'Bob', fioUsername: null,
      })
